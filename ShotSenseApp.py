...
def load_data():
    if not os.path.exists("shot_events.json"):
        return pd.DataFrame()
...
def save_live_event(result, session_name="Live Session"):
    ...
    hoop_center_x = HOOP_ROI[0] + HOOP_ROI[2] // 2
    hoop_center_y = HOOP_ROI[1] + HOOP_ROI[3] // 2
    ...
    event = {
        ...
        "hoop_roi": list(HOOP_ROI),
        "net_roi": list(NET_ROI),
        "green_box_max_area": 0.0,
        "green_box_hit_frames": 0,
...
class LiveVideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        motion_in_hoop = len(large_hoop) > 0
        ...
        if self.motion_frames >= 3 and (current_time - self.last_event_time) > 2.0:
            ...
            save_live_event(result, session_name="Live Session")
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("Upload Basketball Video", type=["mp4", "mov", "m4v"])

    if uploaded_file is not None:
        with open("uploaded_video.mp4", "wb") as f:
            f.write(uploaded_file.read())

        with open("current_session_name.txt", "w") as f:
            f.write(session_name.strip())
    ...
    if st.button("Run Shot Detection"):
        if not os.path.exists("uploaded_video.mp4"):
            ...
            with st.spinner("Processing video..."):
                exit_code = os.system("python3 week16.py")

            if exit_code == 0:
                st.success("Shot detection complete.")
                st.rerun()
            else:
                st.error("Shot detection failed. Check your script and ROI settings.")
...
# ================= LIVE PAGE =================
    ...
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.subheader("Fixed Camera Live View")
    st.write("This opens a live camera feed inside your ShotSense dashboard.")
    st.info("If you are using your phone, open the Streamlit network URL on the same Wi-Fi and allow camera access in the browser.")
